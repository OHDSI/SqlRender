...
test_that("translateSQL sql server -> Oracle INT in conditional create table", {
    sql <- translateSql("IF OBJECT_ID('test_a', 'U') IS NULL CREATE TABLE test_a (	x INT);",
    targetDialect = "oracle")$sql
    expect_equal_ignore_spaces(sql, "BEGIN\n  EXECUTE IMMEDIATE 'CREATE TABLE test_a  (x NUMBER(10))';\nEXCEPTION\n  WHEN OTHERS THEN\n    IF SQLCODE != -955 THEN\n      RAISE;\n    END IF;\nEND;")
})
