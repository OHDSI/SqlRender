...
test_that("translate sql server -> spark convert date", {
  sql <- translate("SELECT convert(date, '2019-01-01')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select TO_DATE('2019-01-01', 'yyyymmdd')")
})
...
test_that("translate sql server -> spark dateadd", {
  sql <- translate("SELECT dateadd(second, 1, '2019-01-01 00:00:00')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select ('2019-01-01 00:00:00' || 1*INTERVAL'1 second')")
  
  sql <- translate("SELECT dateadd(minute, 1, '2019-01-01 00:00:00')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select ('2019-01-01 00:00:00' || 1*INTERVAL'1 minute')")
  
  sql <- translate("SELECT dateadd(hour, 1, '2019-01-01 00:00:00')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select ('2019-01-01 00:00:00' || 1*INTERVAL'1 hour')")
  ...
  sql <- translate("SELECT dateadd(m, 1, '2019-01-01')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select ('2019-01-01' || 1*INTERVAL'1 month')")
  ...
  sql <- translate("SELECT dateadd(yy, 1, '2019-01-01')",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select ('2019-01-01' || 1*INTERVAL'1 year')")
...
test_that("translate sql server -> spark convert date", {
  sql <- translate("select convert(varchar,'2019-01-01',112)",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select TO_CHAR('2019-01-01', 'YYYYMMDD')")
})
...
test_that("translate sql server -> spark isnumeric", {
  sql <- translate("select ISNUMERIC(x)",
                   targetDialect = "spark")
  expect_equal_ignore_spaces(sql, "select CASE WHEN (CAST(x AS string) RLIKE '^([0-9]+\\.?[0-9]*|\\.[0-9]+)$') THEN 1 ELSE 0 END")
})
